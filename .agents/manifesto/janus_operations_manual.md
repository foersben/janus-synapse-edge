...
* **Engine B (Reflexive Execution 7B):** **4.5 GB** VRAM static fence. Vision encoder projection is offloaded to system CPU/RAM to protect the VRAM limit.
...
1. **OS Disk (30 GB):** Formatted with default `volblocksize=16K` for Talos system logs and container states.
2. **AI Data Disk (1.2 TB):** Formatted strictly with `volblocksize=1M` for weights and Letta's asynchronous `io_uring` database streams.
