...
Because the Proxmox VE hypervisor is secured at the bare-metal level via block-level LUKS Encryption, it is fundamentally impossible for the primary server to achieve a fully automated boot sequence. The recovery protocol mandates a strict "Human-in-the-Loop" phase to satisfy cryptographic requirements, transitioning thereafter into fully automated orchestration:

1.  **The Wake-on-LAN Trigger:** Once grid power stabilizes, the low-power, UPS-backed Intel NUC node boots up, verifies internal network health, and transmits a hardware **Wake-on-LAN (WoL)** "magic packet" over the Netgear switch. This triggers the physical boot sequence of the main server's motherboard.
2.  **The Cryptographic Bridge:** The compute node halts in its `initramfs` RAM disk, exposing a secure Dropbear SSH utility. The operator (or a secured external jump-box) executes a remote unlock script, securely injecting the LUKS key via an SSH Agent connection.
3.  **The Automated Handoff:** Once the hypervisor decrypts the block layer and mounts the $1\text{ MB}$ recordsize ZFS pool, the Talos Linux VMs are initialized. K3s starts up, loads the active cluster configuration, and automatically schedules the **Rust Janitor Daemon**.
...
sequenceDiagram
    ...
    Janitor->>ClickHouse: Pull recent transaction logs (last 5 min)
...
2.  **Letta Cognitive State Reconstruction:** Because the primary compute node is stateless, its active Letta cognitive state might have been corrupted or lost during the power failure. The Janitor queries ClickHouse on the NUC, pulling the last **5 minutes of transaction logs** recorded before the outage.
