...
title: "Disaster Recovery & Automated Healing"
dependencies:
  - ref: docs/🛠️ Preparation The Safe-Stack Configuration.md
exports: {}
tags: [Resilience, Backup, Restic, Healing]
---
...
*   **Phase 2: Autonomic State Reconstruction (Zero-Touch):** Once the drive is unlocked, the "Automated Healing" strictly refers to the software and cognitive state. The Rust Janitor Daemon autonomously purges stale file locks and scratch directories. Because the local NVMe only receives persistent batched writes during the nocturnal "Dreaming" phase, the Janitor queries the ClickHouse NUC database for **all OTLP transaction logs generated since the timestamp of the last successful daily ZFS flush**. It replays this entire delta to populate the local LanceDB vector-graph databases (Recall & Archival memory) embedded in the Rig Orchestrator on VM 2, perfectly reconstructing the agent's cognitive state without data loss.
...
sequenceDiagram
    ...
    participant LocalDisk as ZFS Filesystem
    ...
    Note over Janitor,LocalDisk: WASM Scratch Purge Phase
    Janitor->>LocalDisk: Purge orphaned WASM scratch directories
    Janitor->>LocalDisk: Release stale ZFS file locks
    Note right of LocalDisk: Complete filesystem sanitization<br/>in microseconds.
    ...
    Janitor->>Rig: Replay transaction logs into local LanceDB (VM 2)
    ...
    Rig->>Rig: Spin up fresh WASM sandbox & Resume execution
```

1. **WASM Scratch Directory Purge:** Because a hard power failure wipes all volatile RAM, there are no lingering ghost processes to kill. However, an abrupt crash leaves behind orphaned `.lock` files and corrupted WASM scratch directories on the non-volatile ZFS storage. The Janitor executes a sweeping `rm -rf` on all temporary execution namespaces, restoring a pristine filesystem state in microseconds.
2. **LanceDB State Reconstruction:** Because the primary compute node is transactionally stateless during active hours, its active local LanceDB vector-graph databases on VM 2 might have been corrupted or lost during the power failure. The Janitor queries ClickHouse on the NUC, pulling **all OTLP transaction logs generated since the timestamp of the last successful daily ZFS flush** to prevent memory loss.
3. **Transaction Replay:** The Janitor replays the transaction logs directly into the local LanceDB engine on VM 2, reconstructing the agent's virtual **Core Memory**, **Recall Memory**, and **Archival Memory** vector-graph indices to the exact millisecond before the power failure occurred.
4. **Context Injection & Resume:** The Janitor sends an API callback to the **Rig Orchestrator**, resetting the task's state in the transaction queue to `QUEUED`, and injects a recovery context trace directly into the agent's volatile memory:
    > *"System recovered from ungraceful power failure. Task 104 was interrupted. Stale file locks and orphaned scratch directories have been purged. Local Letta state has been successfully replayed and verified. Resume execution from the last safe checkpoint."*
