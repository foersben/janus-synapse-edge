...
Because the Proxmox host is secured via Zero-Trust ZFS Native Encryption, it is fundamentally impossible for the primary server to achieve an unattended, fully automated recovery sequence. The recovery protocol mandates a strict "Human-in-the-Loop" phase to satisfy cryptographic requirements.
...
2. **The Cryptographic Air-Gap:** The server halts at the pre-boot environment, prompting for the ZFS decryption passphrase. The human operator must physically connect a keyboard and input the high-entropy key.
3. **The Automated Handoff:** Once the hypervisor unlocks the SSD, the Talos VMs boot. K3s initiates, pulling the latest `etcd` snapshot from the Intel NUC (if corruption is detected). Upon establishing a healthy cluster state, the Kubernetes scheduler immediately launches the **Python Janitor Daemon**.
