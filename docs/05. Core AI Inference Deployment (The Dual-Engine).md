...
block-beta
  ...
  A_HOT["Engine A: Hot Neurons (11.0 GB)"]:12
  B_ALL["Engine B: Weights (4.5 GB)"]:6
  OVER["CUDA<br/>(0.5G)"]:2

  RAM_TITLE["System DDR5 Allocation (128GB Total)"]:20

  A_COLD["Engine A:<br/>Cold Neurons<br/>(24GB)"]:5
  K8S["K8s & OS<br/>(12GB)"]:4
  ZFS["ZFS ARC<br/>(8GB)"]:3
  FREE["Unallocated Hardware Buffer (84.0 GB)"]:10
...
!!! warning "Sacrificing GPUDirect Storage (GDS)"
    ...
    However, because the primary FireCuda 530 is secured at the bare-metal level via **LUKS Block Encryption**, the Proxmox kernel strictly controls the I/O path. GPUDirect Storage is therefore topologically impossible in the Janus architecture.
