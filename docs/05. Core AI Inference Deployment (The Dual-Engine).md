...
1. **Static VRAM Fencing:** To guarantee system stability and prevent memory allocation battles between the engines, the SGLang container is launched with a strict static memory limit:
   ...
   ```
   This permanently fences Engine B inside a **$4.5\text{ GB}$ VRAM partition**, leaving the remainder for Engine A and basic system CUDA overhead.
...
* **Performance Impact:** A cold boot of Engine A requires approximately **$4\text{ seconds}$** (significantly down from $12\text{ seconds}$ in standard virtualization setups). Once the model weights are locked into RAM and VRAM via the `mlock` system call, storage performance becomes irrelevant, and token generation speed is unaffected.

---

### 🛠️ Verification and Safety Checklist

!!! safety "Hugepages Configuration"
    The primary Talos guest OS worker must be configured with 2MB hugepages at boot time (`sysctl vm.nr_hugepages=36864`) to allocate contiguous physical memory blocks for llama.cpp, completely preventing virtual memory fragmentation and OS paging pauses.
