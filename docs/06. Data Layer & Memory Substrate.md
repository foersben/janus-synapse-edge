...
1. **The Fold Operation:** When the active **Core Memory** context window reaches a high-water mark (typically 80% capacity), the Letta control loop automatically triggers a "fold." The agent parses the active token space, summarizes historical context, **transmits non-essential dialog steps asynchronously to the ClickHouse OTLP ledger**, and flushes the active context to reclaim token overhead. *(These folded memories are only compiled into the local Recall NVMe database during the nocturnal Dreaming phase to prevent active-hour write amplification).*
...
### 3.2 ZVOL Block Size Optimization

To match the physical page-alignment of large model weights and contiguous Letta memory blocks, the underlying ZFS volume hosting the Talos VM virtual disk must be optimized. Because Proxmox VM disks are ZVOLs (block devices), this must be configured during VM creation.

**Mechanical Impact:** When provisioning the Tensor Worker (VM 1) in Proxmox, you must explicitly set the **Block Size (`volblocksize`) to 1M** in the Hard Disk advanced settings. Matching this 1MB block size to the chunk sizes streamed by `io_uring` reduces ZFS metadata overhead, maximizes the internal queue parallelism of the Seagate FireCuda NVMe controller, and strictly avoids write-amplification during large contiguous Letta flushes.
