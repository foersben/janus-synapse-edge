...
1. **The Fold Operation:** When the active **Core Memory** context window reaches a high-water mark (typically 80% capacity), the Letta control loop automatically triggers a "fold." The agent parses the active token space, summarizes historical context, writes non-essential dialog steps to the local **Recall Memory** database, and flushes the active context to reclaim token overhead.
...
flowchart TD
    ...
    FoldingLogic == "Fold (Flushes context to database)" ==> RecallDB
...
### 3.2 ZFS Recordsize Optimization

To match the physical page-alignment of large model weights and contiguous Letta memory blocks, the underlying ZFS dataset hosting the Talos VM virtual disk is optimized:

```bash
zfs set recordsize=1M <pool>/talos-worker
```

**Mechanical Impact:** Changing the recordsize to $1\text{ MB}$ (matching the chunk sizes streamed by `io_uring`) reduces ZFS metadata overhead, maximizes the internal queue parallelism of the Seagate FireCuda NVMe controller, and avoids write-amplification during large contiguous flushes. This matches the underlying physical flash geometries, extending the lifetime of the primary SSD.
...
!!! warning "No local database writes"
    Except for local vector DB recall queries, active runtime database writes are strictly prohibited on the primary compute node to prevent write-amplification on the primary Seagate FireCuda SSD. All analytical logging must route to ClickHouse on the NUC.
