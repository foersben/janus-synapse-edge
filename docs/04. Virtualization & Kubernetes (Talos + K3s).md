...
* **Storage:** 1.2 TiB (1228 GiB), split into a **30 GiB** OS disk (ZVOL with `16K` block size) and an **1198 GiB** AI Data Disk (ZVOL with `1M` block size). This AI Data Disk is mounted strictly to VM 1 to host model weights (GGUFs), maximizing sequential read performance.
  *Architectural Note on Capacity:* A physical 2 TB Seagate FireCuda drive yields exactly 1862 GiB of formatted, usable space. With 1228 GiB provisioned to the Tensor Worker (VM 1), 100 GiB to the Brain Worker (VM 2), and 100 GiB reserved for the Proxmox hypervisor substrate, the system consumes 1428 GiB. This leaves an exact unallocated ZFS buffer of **434 GiB**. This 434 GiB buffer is strictly maintained to preserve high SSD write endurance and accommodate future Talos system snapshotting without saturating the drive.
...
Because Engine A, Engine B, and Whisper STT operate as discrete Pods that all require hardware acceleration, the standard NVIDIA device plugin will cause a scheduling deadlock. To permit all three pods to share the RTX 5070 Ti simultaneously, a Time-Slicing `ConfigMap` must be deployed to the cluster prior to inference initialization:
  ...
  config.yaml: |
    ...
    sharing:
      timeSlicing:
        resources:
          - name: nvidia.com/gpu
            replicas: 3 # Exposes the single GPU as three logical devices to schedule Engine A, Engine B, and Whisper STT
```
...
* **The Talos Guest OS Hook:** In the `talos-machineconfig.yaml` for VM 1 (Tensor Worker), inject the hugepage reservation parameters into the kernel command line to match the 84GB host allocation:
  ...
  This ensures that the Talos guest OS immediately registers and allocates all 84GB of contiguous 1GB pages at boot time, preventing memory fragmentation and kernel panic risks.
