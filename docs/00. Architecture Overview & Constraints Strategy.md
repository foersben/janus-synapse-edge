...
title: Architecture Overview & Constraints Strategy
dependencies: []
exports:
  hardware:
    cpu_threads_total: 28
    ram_gb_total: 128
    vram_gb_total: 16
    storage_gib_total: 1862
  power:
    cpu_pl_limit_watts: 180
tags: [Hardware, Constraints, Topology]
---
...
| **Chassis** | Jonsbo Z20 (Micro-ATX, 20-liter) | Highly restricted thermal dissipation; CPU capped to 180W to mitigate the factory 280W+ peak. |
...
### 2.2 Engine B: The Multimodal Reflex Agent (llama.cpp / Qwen2-VL)

Engine B is engineered for low-latency, high-frequency operations such as tool-calling, external API interactions, and immediate user interface responses. Because Engine B must act as a multimodal reflex agent that handles visual DOM-scraping (and cannot use a text-only model like Qwen-2.5-7B which lacks the architecture to process visual data), the specification explicitly mandates a specialized multi-modal 8B parameter model, specifically **Qwen2-VL**.
By unifying the backend on **llama.cpp**, Janus leverages heterogeneous CPU/GPU offloading to safely isolate multi-modal vision encoders into system RAM. The 4-bit quantized text weights are loaded entirely into VRAM to execute text generation natively on the GPU, while the Vision Encoder is offloaded to system CPU/RAM, keeping Engine B's GPU memory footprint strictly within its 4.0GB static VRAM fence (part of the 4.5 GB time-sliced partition shared with the Whisper STT server).

---

## 3. High-Level System Architecture Diagram

The following C4-style architectural diagram illustrates the physical separation and logical routing between the transactionally stateless (during active hours) compute tier and the resilient state-storage tier.
    ...
    subgraph MainCompute["Main Compute Node (Jonsbo Z20 Micro-ATX)"]
        ...
        subgraph TalosVM["Talos Linux VM (Immutable OS)"]
            ...
            subgraph AI_Pods["AI Inference Pods (ArgoCD Managed)"]
                ...
                EngineB["llama.cpp (Engine B: Qwen2-VL)<br/>4.0GB VRAM (GPU/CPU Split)"]:::software
            end
        end
...
To mitigate data corruption while maintaining high performance, the system is partitioned into transactionally stateless (during active hours) compute and stateful memory telemetry tiers.

1. **The Transactionally Stateless Main Server:** The primary compute node operates as transactionally stateless during active hours (routing all micro-writes to the NUC) to prevent local write-amplification, while its local NVMe hosts the daily-compiled, read-heavy Letta Archival state. Due to its reliance on an immutable operating system (Talos) and declarative GitOps (ArgoCD), the main server's base configuration cannot be corrupted by a sudden power failure. Upon power restoration, the cluster self-heals and pulls the latest declarative state from the repository, while the NUC asynchronously reconstructs the Letta memory state.
