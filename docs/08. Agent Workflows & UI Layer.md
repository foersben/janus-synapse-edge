...
!!! safety "Control Plane Isolation"
    The Rig orchestrator process must be explicitly bound to CPU affinity mask `0x03FF0000` (Threads 16-25) via systemd unit configurations to prevent E-Core thread contamination into P-Core domains and avoid starving the hypervisor's LUKS decryption threads (26-27).
