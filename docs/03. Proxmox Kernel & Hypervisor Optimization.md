...
Proxmox is strictly allocated a **12 GB memory envelope**: 8 GB for the ZFS ARC, and 4 GB for the underlying Debian OS and routing overhead. The remaining 116 GB is passed directly to the virtual machines.

*Note:* Hugepages (for zero-fragmentation memory allocation) are deliberately *not* configured at the Proxmox host layer. Doing so would permanently lock physical memory pages away from the dynamic allocation pools of the VMs.

### 3.2 Deterministic CPU Shielding (Core Pinning)
...
3. Reboot the host. The hypervisor is now physically walled off, utilizing only these two E-cores for ZFS encryption overhead and 2.5G network I/O.
