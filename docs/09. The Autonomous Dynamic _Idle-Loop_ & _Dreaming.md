...
title: "The Autonomous Dynamic Idle-Loop & Dreaming"
dependencies: []
exports:
  priority_human_in_loop: 1000
  priority_background_yield: 100
  micro_downtime_trigger_min: 45
tags: [Preemption, Letta, GEPA, KubernetesScheduler]
---
...
3.  **Manual Overrides:** Triggering an "Away Mode" webhook via the LobeChat / Open-WebUI interface instantly forces the cluster into deep-training mode.
...
!!! success "Kubernetes PriorityClass Partitioning"
    *   **System-Critical (Priority: 1000):** Assigned to all core interactive components—including the TensorZero Gateway, Engine A, Engine B, Letta memory services, and the LobeChat / Open-WebUI frontend.
...
If the GPU is operating at 100% load during a prompt evolution loop, and the human operator inputs a manual query into LobeChat / Open-WebUI:

1.  The TensorZero Gateway intercepts the incoming human prompt.
2.  The TensorZero Gateway registers the System-Critical priority and dispatches an internal gRPC interrupt to the Rig control plane. Because Wasmtime runs embedded within the parent Rust process, the Tokio async runtime triggers a thread-level `Wasmtime::InterruptHandle`, instantly terminating the background GEPA sandbox execution and yielding the E-Core CPU threads in microseconds without killing the orchestrator pod.
