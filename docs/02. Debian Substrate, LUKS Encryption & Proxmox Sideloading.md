...
*Note: The client connects utilizing an ED25519 key stored solely in volatile memory via a password manager's SSH Agent (e.g., KeePassXC). The client executes `/bin/remote-unlock`, unlocking the drive and intentionally severing the connection as the main OS takes over.*
