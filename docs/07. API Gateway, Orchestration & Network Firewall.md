...
* **VRAM Fencing (Context Capping):** TensorZero enforces hard input payload size limits mapped exactly to Engine B's `8192` context window. Because `llama.cpp` pre-allocates its KV-Cache statically upon boot, TensorZero acts as the upstream shield, instantly rejecting any payload that exceeds 8,000 tokens. This guarantees Engine B never attempts to page beyond its strict $4.5\text{ GB}$ VRAM boundary, safely preserving the $11\text{ GB}$ allocation for Engine A.
...
*   **The Whisper STT Bridge:** Because Engine B (Qwen2-VL) is strictly a Vision-Language model, it cannot ingest audio waveforms natively. A dedicated, highly quantized `whisper.cpp` pod runs on the E-Cores. It intercepts the WebRTC audio, executes near-instantaneous Speech-to-Text translation, and streams the text tokens directly to Engine B for reasoning. To prevent E-Core starvation, the `whisper.cpp` pod must be strictly limited via Kubernetes manifests to `resources.limits.cpu: 2000m` (2 vCPUs).
...
The browser rendering pod (e.g., Playwright/Puppeteer) is highly CPU-intensive and must never compete with the deep-reasoning inference engines. Therefore, the `outbound-dmz` Playwright pod is explicitly scheduled to the **Brain Worker (VM 2)**. By confining web rendering to the physical E-Cores (Threads 16-25), the P-Cores remain mathematically isolated for Engine A's speculative verification passes.
...
1. **The DOM-to-Markdown Rule:** The Playwright DMZ pod must first pass all target URLs through a sanitization script (e.g., Mozilla's `Readability.js` or an HTML-to-Markdown parser). This strips CSS, ad-bloat, and visual formatting, returning pure, clean Markdown to Engine B as a standard text payload.
...
flowchart TD
    ...
    DMZ[outbound-dmz : Playwright Pod]
