...
*   **Native Audio Processing:** Engine B (llama.cpp) loads a native speech-to-speech multimodal model, processing raw audio waveforms directly.
*   **The Interrupt Protocol:** When a user speaks over the AI, the WebRTC stream routes the interruption signal to llama.cpp. By terminating the active inference slot request, the engine halts token generation in under $50\text{ ms}$ and immediately pivots to listening for the new auditory command.
