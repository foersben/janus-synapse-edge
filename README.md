...
| :--- | :--- | :--- |
| **Compute** | Intel i7-14700K (20 Cores) | Hard-capped to 180W. 8 P-Cores pinned for speculative verification; 12 E-Cores for Rig, WASM, and gateway control planes. |
| **RAM** | 128GB DDR5 Crucial | Talos Guest-OS Hugepages (56GB locked) for llama.cpp weights and Letta archival paging. |
...
| **CrewAI / Agent Zero** $\rightarrow$ **Rust-native Rig Framework** | Python orchestration stacks consumed $2\text{ GB+}$ of system memory and introduced stop-the-world garbage collection pauses, stalling the reflex loops. | Pinned strictly to the **12 Intel E-Cores (Threads 16–27)**, the entire agent control plane executes inside a **$<150\text{ MB}$ RSS memory fence**. |
